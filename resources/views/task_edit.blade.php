            ...
            <div class="card-body">
                <form action="{{ route('tasks_store') }}" method="POST">
                    ...
                    <div class="form-group py-4">
                        ...
                        <textarea class="form-control" required id="description" name="description" rows="3">{{ $task->description }}</textarea>
                    </div>
                    <div class="form-group py-4">
                        <label for="task_type">Task Type</label><br>
                        <input type="radio" id="one_time" class="form-check-input"
                            @if ($task->recurring == 0) checked @endif name="task_type" value="one_time">

                        <label for="one_time">One Time Task</label><br>
                        <input type="radio" id="recurring" class="form-check-input"
                            @if ($task->recurring == 1) checked @endif name="task_type" value="recurring">
                        <label for="recurring">Recurring Task</label>
                    </div>
                    <div id="recurring_fields" class=" py-4" style="display: none;">
                        <div class="form-group">
                            <label for="start_date">Start Date *</label>
                            <input type="date" class="form-control" id="start_date" value="{{ $task->start_date }}"
                                name="start_date">
                        </div>
                        <div class="form-group py-4" id="end_date_group">
                        ...
                        <div class="form-group py-4">
                            ...
                            <select name="frequency" class="form-control" id="frequency">
                                ...
                                <option value="Every 2 Weeks" @if ($task->frequency == 'Every 2 Weeks') selected @endif>Every 2
                                    Weeks</option>
                                <option value="Monthly" @if ($task->frequency == 'Monthly') selected @endif>Monthly</option>
                                <option value="Every 2 Months" @if ($task->frequency == 'Every 2 Months') selected @endif>Every 2
                                    Months</option>
                                <option value="Quarterly" @if ($task->frequency == 'Quarterly') selected @endif>Quarterly
                                </option>
                                <option value="Every 6 Months" @if ($task->frequency == 'Every 6 Months') selected @endif>Every 6
                                    Months</option>
                                <option value="Annually" @if ($task->frequency == 'Annually') selected @endif>Annually</option>
                            </select>
                        </div>
                        <div class="form-group py-4">
                            <label for="days_until_due">Number of days until due</label>
                            <input type="number" class="form-control" value="{{ $task->days_until_due }}"
                                id="days_until_due" name="days_until_due">
                        </div>
                        <div class="form-check py-4">
                            <input type="checkbox" class="form-check-input"
                                @if ($task->repeat_forever == 'on') checked @endif id="repeat_forever"
                                name="repeat_forever">
                    ...
                    <div class="form-check py-4">
                        ...
                        <input type="checkbox" class="form-check-input" @if ($task->notify_assignees == 'on') checked @endif id="notify_assignees" name="notify_assignees">
                    </div>
                    <div class="form-group py-4">
